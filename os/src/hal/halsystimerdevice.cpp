...
u32 caHalSysTimerDevice::Open(caIDeviceConfigure * setup,
    ...
    if (conf != NULL) {
        res = link->hll_config(conf->tick_ps, conf->clock_ps, conf->prescaler_ps, conf->irq_ps);
        if (res) {
            isOpen++;
...
#define WRITE_TIMER_SETUP 4*sizeof(u32)

u32 caHalSysTimerDevice::Write(caDeviceHandle *port) {
    u32 res = deviceError::no_error;
    LOG(caLog, device) << " in : isOpen = " << isOpen << caEnd::endl;
    if (port->wrSize == WRITE_TIMER_SETUP) {
        u32 * ptr = (u32*) port->wrBuff;
        res = hal_llc_time.hll_settime(ptr[0], ptr[1], ptr[2], ptr[3]);
        port->writed += WRITE_TIMER_SETUP;
        port->wrBuff += WRITE_TIMER_SETUP;
        port->wrSize -= WRITE_TIMER_SETUP;
...
u32 caHalSysTimerDevice::Read(caDeviceHandle *port) {
    ...
    if (port->rdSize > 0) {
        ...
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_day();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_hour();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_min();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_sec();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_ms();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_us();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_ns();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        if (port->rdSize >= sizeof (u32)) {
            *ptr++ = hal_llc_time.hll_ps();
            port->rdSize -= sizeof (u32);
            port->readed += sizeof (u32);
        }
        port->tLast = hal_llc_time.hll_tick();
...
u32 caHalSysTimerDevice::IoCtrl(caDeviceHandle *port,
    ...
    switch (in->command) {
        ...
        case caSysTimerDeviceCtrl::IoCtrlDirect::sysTimerListHardware:
            ...
            if (in->ss != NULL) {
                in->ss->Clear();
                res = hal_llc_time.hll_dump(in->ss);
            } else {
...
u32 caHalSysTimerDevice::IrqService1(u8 *, s_t size, s_t &) {
...
u32 caHalSysTimerDevice::IrqService2(u8 *, s_t size, s_t &) {
